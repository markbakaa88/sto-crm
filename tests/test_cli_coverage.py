...
class TestCliCoverage(unittest.TestCase):
    ...
    @patch("time.sleep")
    def test_cli_shutdown_double_call(
        self, mock_sleep, mock_web_open, mock_init_db, mock_create_server
    ):
    ...
    @patch("webbrowser.open")
    @patch("time.sleep")
    def test_cli_threading_mismatch(
        self, mock_sleep, mock_web_open, mock_init_db, mock_create_server
    ):
    ...
    def test_cli_sigterm_missing(
        ...
        with (
            ...
            registered_sigs = [
                call[0][0] for call in mock_sig_func.call_args_list if call[0]
            ]
    ...
    @patch("webbrowser.open")
    @patch("time.sleep")
    def test_cli_normalize_db_path_directory(
        self, mock_sleep, mock_web_open, mock_init_db, mock_create_server
    ):
