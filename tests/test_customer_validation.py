...
def test_customer_phone_masking(crm_server):
    ...
    with sync_playwright() as p:
        ...
        assert not page.evaluate(
            "document.getElementById('customer_phone').classList.contains('valid')"
        )
        assert not page.evaluate(
            "document.getElementById('customer_phone').classList.contains('invalid')"
        )
...
def test_customer_email_validation(crm_server):
    ...
    with sync_playwright() as p:
        ...
        assert not page.evaluate(
            "document.getElementById('customer_email').classList.contains('valid')"
        )
        assert not page.evaluate(
            "document.getElementById('customer_email').classList.contains('invalid')"
        )
        ...
        error_msg = page.locator(
            ".field:has(#customer_email) .field-error"
        ).text_content()
        ...
        success_msg = page.locator(
            ".field:has(#customer_email) .field-success"
        ).text_content()
