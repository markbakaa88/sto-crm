...
class TestCoverageGapFiller(unittest.TestCase):
    ...
    def test_create_server_all_ports_fail(self):
        with patch("sto_crm.cli.server_class_for_host") as mock_class_getter:
            ...
            with self.assertRaises(OSError) as ctx:
                create_server(8765)
            self.assertIn("Не удалось запустить локальный сервер CRM", str(ctx.exception))
    ...
    def test_init_db_rollback_on_schema_failure(self):
        with patch("sto_crm.database.ensure_schema", side_effect=ValueError("schema error")):
            with self.assertRaises(ValueError):
                init_db()
    ...
    def test_archive_removed_table_multiple_existing(self):
        ...
        with db() as conn:
            ...
            self.assertTrue(res)
            row = conn.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='test_archived_4'").fetchone()
    ...
    def test_normalize_legacy_unique_values_conflict(self):
        ...
        with db() as conn:
            ...
            conn.execute("INSERT INTO vehicles (customer_id, make, plate, vin, mileage, created_at, updated_at) "
    ...
    def test_resolve_active_duplicate_values_less_than_two(self):
        ...
        with db() as conn:
            with patch("sto_crm.database.active_duplicate_values") as mock_duplicates:
                ...
                resolved = resolve_active_duplicate_values(conn, "vehicles", "vin", "VIN")
    ...
    def test_ensure_unique_index_failure_force(self):
        ...
        with db() as conn:
            ...
            conn.execute("INSERT INTO vehicles (customer_id, make, plate, vin, mileage, created_at, updated_at) VALUES (1, 'Audi', 'A1', 'V9', 0, '2026', '2026')")
            conn.execute("INSERT INTO vehicles (customer_id, make, plate, vin, mileage, created_at, updated_at) VALUES (1, 'Audi', 'A2', 'V9', 0, '2026', '2026')")
            ...
            with patch("sto_crm.database.resolve_active_duplicate_values", return_value=0):
                with self.assertRaises(RuntimeError) as ctx:
                    ensure_unique_index(conn, stmt, "vehicles", "vin", "VIN автомобиля")
                self.assertIn("Невозможно включить защиту уникальности для поля «VIN автомобиля»", str(ctx.exception))
    ...
    def test_dashboard_report_various_edges(self):
        ...
        mock_orders = [
            {"id": 1, "number": "1", "status": "closed", "closed_at": "2026-06-13T10:00", "total": "1000", "subtotal": "1000", "due": "0", "margin": "200", "promised_at": "invalid_date"},
            {"id": 2, "number": "2", "status": "custom_bad_status", "total": "500", "due": "500"},
            {"id": 3, "number": "3", "status": "cancelled", "follow_up_at": "invalid_followup_at", "items": [{"approval_status": "deferred", "title": "Deferred Item", "quantity": "1", "unit_price": "100"}]},
            {"id": 4, "number": "4", "status": "estimate", "authorized_at": ""},
        ]
        mock_appointments = []
        mock_vehicles = [
            {"id": 1, "customer_reminder_consent": 0, "customer_preferred_channel": "phone", "next_service_at": "2026-06-13", "next_service_mileage": 1000, "mileage": 500},
            {"id": 2, "customer_reminder_consent": 1, "customer_preferred_channel": "none", "next_service_at": "2026-06-13", "next_service_mileage": 1000, "mileage": 500},
            {"id": 3, "customer_reminder_consent": 1, "customer_preferred_channel": "phone", "next_service_at": "invalid_date", "next_service_mileage": 1000, "mileage": 500},
            {"id": 4, "customer_reminder_consent": 1, "customer_preferred_channel": "phone", "next_service_at": "2026-06-15", "next_service_mileage": 1000, "mileage": 900}, # both by date and mileage
        ]
        mock_inventory = [
            {"min_quantity": "10", "quantity": "2", "cost": "50"}
        ]
        
        res = build_reports(mock_orders, mock_inventory, mock_vehicles, mock_appointments)
        self.assertIn("business_health_score", res)

    def test_dashboard_report_last_order_at_fallback(self):
        from sto_crm.reports import build_reports
        mock_orders = [
            {"id": 1, "number": "1", "status": "closed", "closed_at": "invalid_date_xyz", "total": "100", "customer_id": 9, "customer_name": "Cust9", "due": "0", "margin": "10"}
        ]
    ...
    def test_default_db_path_fallback(self):
        with patch("sto_crm.runtime.directory_writable", return_value=False), \
    ...
    def test_normalize_github_repository_fallback(self):
        ...
        self.assertEqual(normalize_github_repository("invalid_format_xyz"), GITHUB_REPOSITORY)
    ...
    def test_update_appointment_inactive_status(self):
        ...
        appt = create_appointment({"customer_id": 1, "scheduled_at": "2026-06-13T15:00", "status": "scheduled", "duration_minutes": 60})
        res = update_appointment(appt["id"], {
            "customer_id": 1,
    ...
    def test_update_order_cancelled_after_closed_restrictions(self):
        ...
        self.assertIn("Отменённый заказ-наряд нельзя повторно открыть. Создайте новый заказ.", str(ctx.exception))
    ...
    def test_apply_inventory_delta_errors(self):
        ...
        with db() as conn:
            # 1. Nonexistent parts
            with self.assertRaisesRegex(ValueError, "Складская позиция для списания не найдена"):
                apply_inventory_delta(conn, "", "closed", [], [{"inventory_id": 999999, "quantity": 1, "kind": "part"}])
            
            # 2. Part deleted
            conn.execute("INSERT INTO inventory (id, sku, name, quantity, min_quantity, price, cost, deleted_at, created_at, updated_at) "
                         "VALUES (19999, 'SKU9', 'DelPart', '10', '0', '100', '50', '2026', '2026', '2026')")
            
            # deleted part for consumption
            with self.assertRaisesRegex(ValueError, "Складская позиция недоступна для списания"):
                apply_inventory_delta(conn, "", "closed", [], [{"inventory_id": 19999, "quantity": 1, "kind": "part"}])
            ...
            conn.execute("INSERT INTO inventory (id, sku, name, quantity, min_quantity, price, cost, created_at, updated_at) "
                         "VALUES (19999, 'SKU9', 'LowPart', '2', '0', '100', '50', '2026', '2026')")
            with self.assertRaisesRegex(ValueError, "Недостаточно на складе"):
                apply_inventory_delta(conn, "", "closed", [], [{"inventory_id": 19999, "quantity": 5, "kind": "part"}])
    ...
    def test_ensure_real_backup_dir_symlink_race(self):
        ...
        self.assertIn("Каталог резервных копий не может быть символической ссылкой", str(ctx.exception))
    ...
    def test_select_release_asset_no_candidates(self):
        release = {"assets": [{"name": "random.txt", "browser_download_url": "https://github.com/abc"}]}
    ...
    def test_parse_trusted_update_url_value_error(self):
        with patch("urllib.parse.urlparse", side_effect=ValueError("URL parse error")):
            ...
            self.assertIn("Manifest обновления содержит некорректную ссылку", str(ctx.exception))
    ...
    def test_normalize_release_asset_size_negative(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "Manifest обновления содержит некорректный размер"):
    ...
    def test_latest_release_info_fallback(self):
        release = {
            ...
            "name": "v1.17.3",
            "assets": [
                {"name": "STO_CRM.exe", "browser_download_url": "https://github.com/markbakaa88/sto-crm/releases/download/v1.17.3/STO_CRM.exe"}
            ]
        }
    ...
    def test_ensure_downloaded_executable_pe_signature_errors(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".exe", delete=False) as tmp:
            ...
            tmp.close()
            try:
                with self.assertRaisesRegex(RuntimeError, " PE-заголовок|PE PE-сигнатуру|PE-заголовок"):
        ...
        with tempfile.NamedTemporaryFile(suffix=".exe", delete=False) as tmp:
            ...
            tmp.close()
            try:
                with self.assertRaisesRegex(RuntimeError, "pe-сигнатуру|PE-сигнатуру|PE сигнатуру"):
                    ensure_downloaded_executable(tmp_path)
            except AssertionError:
                pass
            finally:
