...
class TestValidationExtra(unittest.TestCase):
    ...
    def test_validate_order_item_invalid_approval_status(self):
        ...
        try:
            ...
            with self.assertRaisesRegex(
                ValueError, "Некорректный статус согласования позиции"
            ):
                validate_order(conn, payload)
        finally:
            conn.close()
    ...
    def test_ensure_vehicle_belongs_to_customer_deleted_or_missing(self):
        ...
        conn.execute(
            "INSERT INTO vehicles (id, customer_id, deleted_at) VALUES (1, 1, '2026-06-12T12:00:00')"
        )
        try:
            with self.assertRaisesRegex(ValueError, "Выберите действующий автомобиль"):
            ...
            res = ensure_vehicle_belongs_to_customer(
                conn, 1, 1, allow_deleted_vehicle_id=1
            )
            ...
            with self.assertRaisesRegex(
                ...
                ensure_vehicle_belongs_to_customer(
                    conn, 1, 2, allow_deleted_vehicle_id=1
                )
        finally:
            conn.close()
    ...
    def test_ensure_no_appointment_conflict_non_positive_duration(self):
        ...
        try:
            with self.assertRaisesRegex(
                ValueError, "Длительность записи должна быть больше нуля"
            ):
    ...
    def test_timezone_mix_appointment_conflict_combinations(self):
        ...
        local_aware_non_conflict_dt = (naive_dt + timedelta(hours=2)).replace(
            tzinfo=local_tz
        )
        ensure_no_appointment_conflict(
            conn, local_aware_non_conflict_dt.isoformat(), 30
        )
    ...
    def test_validate_order_item_various_errors(self):
        ...
        try:
            with self.assertRaisesRegex(
                ValueError, "Позиция заказ-наряда должна быть JSON-объектом"
            ):
                validate_order_item(conn, "not-a-dict")

            with self.assertRaisesRegex(
                ValueError, "Некорректный тип позиции заказ-наряда"
            ):
                validate_order_item(conn, {"kind": "invalid"})

            with self.assertRaisesRegex(
                ValueError, "Укажите наименование запчасти или работы"
            ):
                validate_order_item(conn, {"kind": "service", "quantity": 1})

            res = validate_order_item(
                conn,
                {
                    "kind": "service",
                    "inventory_id": -5,
            ...
            res_part_2 = validate_order_item(
                conn,
                {
                    "kind": "part",
                    "inventory_id": 1,
    ...
    def test_ensure_unique_active_value_checks(self):
        ...
        conn.row_factory = sqlite3.Row
        conn.execute(
            "CREATE TABLE inventory (id INTEGER PRIMARY KEY, sku TEXT, deleted_at TEXT)"
        )
        conn.execute(
            "INSERT INTO inventory (id, sku, deleted_at) VALUES (10, 'SKU-EXISTING', NULL)"
        )
        try:
            ...
            with self.assertRaisesRegex(ValueError, "error message"):
                ensure_unique_active_value(
                    conn, "inventory", "sku", "SKU-existing", "error message"
                )

            ensure_unique_active_value(
                conn, "inventory", "sku", "SKU-existing", "error message", record_id=10
            )
        finally:
            conn.close()
    ...
    def test_active_appointment_counters(self):
        ...
        conn.execute(
            "INSERT INTO appointments VALUES (4, 1, 10, 'scheduled', '2026-06-12')"
        )
        try:
