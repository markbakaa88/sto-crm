...
class TestHttpServerExtra(unittest.TestCase):
    ...
    def test_invalid_api_routes(self):
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            self.assertEqual(response.headers.get("Allow"), "GET, HEAD, POST, PUT, DELETE, OPTIONS")
    ...
    def test_unsupported_method_permission_error(self, mock_handle):
        ...
        setattr(handler, "headers", FakeHeaders({"Host": "127.0.0.1:8080"}))
        
        with patch.object(handler, "validate_local_request_context", side_effect=PermissionError("Forbidden context")):
    ...
    def test_shutdown_and_update_reboot(self, mock_handle):
        ...
        handler = CRMHandler(mock_request, ("127.0.0.1", 12345), mock_server)
        handler.request_version = "HTTP/1.1"
        setattr(handler, "headers", FakeHeaders({
            "Host": "127.0.0.1:8080",
            "X-CRM-Access-Token": "test_access_token",
            "X-CSRF-Token": "test_csrf_token",
            "Content-Type": "application/json",
        ...
        with patch("sto_crm.http_server.CRMServer", mock_server_class):
            with patch("sto_crm.http_server.install_update_from_github", return_value={"backup": {"filename": "foo.db", "display_path": "foo/display"}, "updated": True}):
                with patch("threading.Timer") as mock_timer:
                    with patch.object(handler, "send_json") as mock_send_json:
                        ...
                        self.assertEqual(res_payload.get("backup"), {"filename": "foo.db", "display_path": "foo/display"})
                        self.assertTrue(res_payload.get("updated"))

        # Test update install else path (isinstance not CRMServer)
        mock_server.graceful_shutdown_flag = False
        with patch("sto_crm.http_server.install_update_from_github", return_value={"updated": True}):
            with patch("threading.Timer") as mock_timer:
                handler.handle_request("POST")
                mock_timer.assert_called_once()
                self.assertTrue(mock_server.graceful_shutdown_flag)
    ...
    def test_handler_validation_details(self, mock_handle):
        ...
        mock_headers_te.get.side_effect = lambda k, default=None: "chunked" if k == "Transfer-Encoding" else default
        setattr(handler, "headers", mock_headers_te)
        ...
        mock_headers_cl.get.side_effect = lambda k, default=None: "10" if k == "Content-Length" else default
        mock_headers_cl.get_all.side_effect = get_all_side_effect
        setattr(handler, "headers", mock_headers_cl)
        with self.assertRaises(ValueError) as ctx:
            handler.reject_ambiguous_body_framing()
        ...
        self.assertIn("Запрос отклонен: внешний сайт не имеет доступа", str(ctx.exception))
        ...
        with self.assertRaises(PermissionError) as ctx:
            handler.validate_local_request_context()
        self.assertIn("Запрос отклонен: внешний источник не имеет доступа", str(ctx.exception))
        ...
        self.assertIn("Для изменений требуется Content-Type: application/json.", str(ctx.exception))
        ...
        setattr(handler, "headers", FakeHeaders({
            "Host": "127.0.0.1:8080",
            "Sec-Fetch-Site": "same-origin",
            "Content-Length": str(2_000_000 + 1)
        }))
        with self.assertRaises(ValueError) as ctx:
            handler.validate_mutating_request("POST")
    ...
    def test_json_parsing_and_complexity(self, mock_handle):
        ...
        setattr(handler, "headers", FakeHeaders({"Content-Length": "abc"}))
        with self.assertRaises(ValueError) as ctx:
            handler.read_json()
        self.assertIn("Некорректная длина запроса.", str(ctx.exception))

        # 2. Negative Content-Length
        setattr(handler, "headers", FakeHeaders({"Content-Length": "-10"}))
        with self.assertRaises(ValueError) as ctx:
            handler.read_json()
        self.assertIn("Некорректная длина запроса.", str(ctx.exception))

        # 3. Content-Length too large
        setattr(handler, "headers", FakeHeaders({"Content-Length": str(10 * 1024 * 1024)}))
        with self.assertRaises(ValueError) as ctx:
            handler.read_json()
        self.assertIn("Слишком большой запрос.", str(ctx.exception))

        # 4. Zero Content-Length
        setattr(handler, "headers", FakeHeaders({"Content-Length": "0"}))
        with self.assertRaises(ValueError) as ctx:
            handler.read_json()
        self.assertIn("Пустое тело JSON-запроса.", str(ctx.exception))

        # 5. Non-dict JSON (returns a list)
        setattr(handler, "headers", FakeHeaders({"Content-Length": "7"}))
        handler.rfile = MagicMock()
    ...
    def test_direct_exception_handling(self, mock_handle):
        ...
        mock_server.server_port = 8080
        mock_request = MagicMock()
        from sto_crm.http_server import CRMHandler, INTERNAL_ERROR_MESSAGE
        handler = CRMHandler(mock_request, ("127.0.0.1", 12345), mock_server)
        handler.request_version = "HTTP/1.1"
        ...
        with patch.object(handler, "validate_local_request_context", side_effect=BrokenPipeError):
            self.assertIsNone(handler.handle_request("GET"))
            
        # 3. TimeoutError in handle_request
        with patch.object(handler, "validate_local_request_context", side_effect=TimeoutError("timeout")):
            with patch.object(handler, "send_error_json") as mock_send_err:
                handler.handle_request("GET")
                self.assertTrue(handler.close_connection)
                mock_send_err.assert_called_once_with(408, "Тело запроса не получено вовремя.")
                
        # 4. OSError in handle_request with stderr present
        with patch.object(handler, "validate_local_request_context", side_effect=OSError("OS error")):
            with patch.object(handler, "send_error_json") as mock_send_err:
                handler.handle_request("GET")
        ...
        with patch.object(handler, "validate_local_request_context", side_effect=Exception("Generic error")):
            with patch.object(handler, "send_error_json") as mock_send_err:
                handler.handle_request("GET")
                mock_send_err.assert_called_once_with(500, INTERNAL_ERROR_MESSAGE)
